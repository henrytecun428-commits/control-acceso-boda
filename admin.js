...
async function refreshSession() {
  ...
  if (admin) {
    await Promise.all([loadInvitations(), loadSettings()]);
  }
}
...
async function loadInvitations() {
  ...
  body.innerHTML = data.map((inv) => {
    ...
    totalConfirmados += Number(inv.confirmed_guests || 0);
    totalIngresaron += used;
    const url = invitationUrl(inv.code);
    const encodedMessage = encodeURIComponent(`Hola ${inv.name} 💕 Aquí tienes tu invitación: ${url}`);

    return `<tr>
      <td><strong>${clean(inv.name)}</strong></td>
      <td><span class="pill">${clean(inv.code)}</span></td>
      <td>${inv.total_guests}</td>
      <td>${inv.confirmed_guests || 0}</td>
      <td>${used}</td>
      <td>${available}</td>
      <td>${clean(inv.rsvp_status)}</td>
      <td><span class="status ${inv.active ? 'status-on' : 'status-off'}">${inv.active ? 'Activa' : 'Bloqueada'}</span></td>
      <td><div class="actions">
        <button class="small-btn secondary" data-action="copy" data-url="${clean(url)}" type="button">Copiar</button>
        <a class="small-btn secondary" href="https://wa.me/?text=${encodedMessage}" target="_blank" rel="noopener">WhatsApp</a>
        <button class="small-btn secondary" data-action="toggle" data-id="${inv.id}" data-active="${inv.active}" type="button">${inv.active ? 'Bloquear' : 'Activar'}</button>
      </div></td>
    </tr>`;
  }).join('');
  ...
  body.querySelectorAll('[data-action="copy"]').forEach((button) => {
...
async function loadSettings() {
  ...
  const fields = {
    ...
    'venue-name': 'venue_name','venue-address': 'venue_address','maps-url': 'maps_url','waze-url': 'waze_url','parking-text': 'parking_text',
    'dress-code': 'dress_code','gift-text': 'gift_text','gift-url': 'gift_url','hero-image-url': 'hero_image_url'
  };
...
document.querySelector('#save-settings').addEventListener('click', async () => {
  ...
  const payload = {
    id: true,
    couple_names: document.querySelector('#couple-names').value.trim(),
    welcome_text: document.querySelector('#welcome-text').value.trim(),
    event_date: document.querySelector('#event-date').value.trim(),
    reception_time: document.querySelector('#reception-time').value.trim(),
    ceremony_time: document.querySelector('#ceremony-time').value.trim(),
    venue_name: document.querySelector('#venue-name').value.trim(),
    venue_address: document.querySelector('#venue-address').value.trim(),
    maps_url: document.querySelector('#maps-url').value.trim(),
    waze_url: document.querySelector('#waze-url').value.trim(),
    parking_text: document.querySelector('#parking-text').value.trim(),
    dress_code: document.querySelector('#dress-code').value.trim(),
    gift_text: document.querySelector('#gift-text').value.trim(),
    gift_url: document.querySelector('#gift-url').value.trim(),
    hero_image_url: document.querySelector('#hero-image-url').value.trim(),
...
document.querySelector('#create-invitation').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase.rpc('create_invitation', { p_name: name, p_total_guests: total, p_allow_children: allowChildren });
