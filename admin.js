...
function previewImage(previewId, url) {
  ...
  if (!preview) return;
  if (url) {
    preview.innerHTML = `<img src="${clean(url)}" alt="Vista previa">`;
  } else {
    preview.textContent = preview.dataset.placeholder || 'Sin foto';
  }
}
...
async function loadSettings() {
  ...
  previewImage('preview-hero', data.hero_image_url); previewImage('preview-gallery-1', data.gallery_image_1_url); previewImage('preview-gallery-2', data.gallery_image_2_url); previewImage('preview-gallery-3', data.gallery_image_3_url); previewImage('preview-gallery-4', data.gallery_image_4_url);
}
...
async function uploadImage(input) {
  ...
  if (!file) return;
  const state = document.querySelector(`#${input.dataset.state}`);
  const preview = document.querySelector(`#${input.dataset.preview}`);
  const targetField = document.querySelector(`#${input.dataset.field || (input.id === 'file-hero' ? 'hero-image-url' : input.id.replace('file-',''))}`);
  if (!file.type.startsWith('image/')) { if(state) state.textContent='Solo imágenes.'; input.value=''; return; }
  if (file.size > 5 * 1024 * 1024) { if(state) state.textContent='Máximo 5 MB por foto.'; input.value=''; return; }

  if (preview) {
    const localUrl = URL.createObjectURL(file);
    preview.innerHTML = `<img src="${localUrl}" alt="Vista previa">`;
  }
  if (state) state.textContent = 'Subiendo...';

  const ext = (file.name.split('.').pop() || 'jpg').toLowerCase().replace(/[^a-z0-9]/g,'');
  const path = `wedding/${Date.now()}-${crypto.randomUUID()}.${ext}`;
  const { error } = await supabase.storage.from('wedding-media').upload(path, file, { contentType: file.type, cacheControl: '31536000', upsert: false });
  if (error) { if(state) state.textContent=`Error: ${error.message}`; return; }
  const { data } = supabase.storage.from('wedding-media').getPublicUrl(path);
  if (targetField) targetField.value = data.publicUrl;
  if (state) state.textContent = 'Foto lista ✓';
}

// Asociar cada selector de archivo con su campo de URL.
document.querySelectorAll('[data-upload]').forEach((input) => {
  const field = input.dataset.upload === 'hero_image_url' ? 'hero-image-url' : input.dataset.upload.replace('gallery_image_', 'gallery-image-');
...
document.querySelector('#save-settings').addEventListener('click', async () => {
  ...
  const {error}=await supabase.from('wedding_settings').upsert(payload); settingsMessage.textContent=error?error.message:'Cambios guardados correctamente.';
});

document.querySelector('#login').addEventListener('click',async()=>{authMessage.textContent='Entrando...';const email=document.querySelector('#email').value.trim();const password=document.querySelector('#password').value;if(!email||!password){authMessage.textContent='Escribe tu correo y contraseña.';return;}const{error}=await supabase.auth.signInWithPassword({email,password});if(error){authMessage.textContent=error.message;return;}if(!(await isAdmin())){await supabase.auth.signOut();authMessage.textContent='Esta cuenta no tiene permisos de administrador.';return;}await refreshSession();});
...
document.querySelector('#create-invitation').addEventListener('click',async()=>{formMessage.textContent='Generando invitación...';createdBox.classList.add('hidden');const name=document.querySelector('#guest-name').value.trim();const total=Number(document.querySelector('#guest-total').value);const allowChildren=document.querySelector('#allow-children').checked;if(!name||!Number.isInteger(total)||total<1||total>100){formMessage.textContent='Completa nombre y cupos correctamente.';return;}const{data,error}=await supabase.rpc('create_invitation',{p_name:name,p_total_guests:total,p_allow_children:allowChildren});const invitation=Array.isArray(data)?data[0]:data;if(error||!invitation){formMessage.textContent=error?.message||'No se pudo crear la invitación.';return;}const url=invitationUrl(invitation.code);formMessage.textContent='Invitación creada correctamente.';createdBox.classList.remove('hidden');createdBox.innerHTML=`<strong>${clean(invitation.name)}</strong><br><span class="created-code">${clean(invitation.code)}</span><p class="notice">Este código es único y conecta invitación, RSVP, QR y control de entrada.</p><div class="created-actions"><button id="copy-created" class="small-btn secondary" type="button">Copiar enlace</button><a class="small-btn secondary" target="_blank" rel="noopener" href="${clean(url)}">Ver invitación</a><a class="small-btn" target="_blank" rel="noopener" href="https://wa.me/?text=${encodeURIComponent(`Hola ${invitation.name} 💕 Aquí tienes tu invitación: ${url}`)}">Enviar por WhatsApp</a></div>`;document.querySelector('#copy-created').addEventListener('click',async()=>{await navigator.clipboard.writeText(url);document.querySelector('#copy-created').textContent='Enlace copiado';});document.querySelector('#guest-name').value='';document.querySelector('#guest-total').value='2';document.querySelector('#allow-children').checked=false;await loadInvitations();});
